...
class Sp4kModulesCartControllersDisplay extends Sp4kBaseControllerDisplay
{
    public function execute()
    {
        if(JFactory::getUser()->guest){
            //if not logged in show them the login/register screen
            $model = new Sp4kModulesCartModelsGuest(
            ...
            $model = new Sp4kModulesRegistrationModelsCart(
                new  Registry($this->input->getArray())
            );

            $view = new Sp4kModulesRegistrationViewsAttendeesHtml($model);
        }
